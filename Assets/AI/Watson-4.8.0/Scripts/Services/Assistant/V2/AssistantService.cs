...
namespace IBM.Watson.Assistant.V2
{
    public partial class AssistantService : BaseService
    {
        ...
        public bool DeleteSessionMod(Callback<MyRootClass> callback, string assistantId, string sessionId)
        {
            if (callback == null)
                throw new ArgumentNullException("`callback` is required for `DeleteSession`");
            if (string.IsNullOrEmpty(assistantId))
            ...
            RequestObject<MyRootClass> req = new RequestObject<MyRootClass>
            {
                Callback = callback,
                HttpMethod = UnityWebRequest.kHttpVerbDELETE,
                DisableSslVerification = DisableSslVerification
            ...
            req.OnResponse = OnDeleteSessionResponseMod;
        ...
        private void OnDeleteSessionResponseMod(RESTConnector.Request req, RESTConnector.Response resp)
        {
            DetailedResponse<MyRootClass> response = new DetailedResponse<MyRootClass>();
            foreach (KeyValuePair<string, string> kvp in resp.Headers)
            {
                response.Headers.Add(kvp.Key, kvp.Value);
            }
            ...
            {
                string json = Encoding.UTF8.GetString(resp.Data);
                response.Result = (MyRootClass)Pathfinding.Serialization.JsonFx.JsonReader.Deserialize(json, typeof(MyRootClass));
                Log.Error(""," Disable Test ",response.Result);
               // response.Result = JsonConvert.DeserializeObject<object>(json);
                response.Response = json;
            }
            catch (Exception e)
           ...
           if (((RequestObject<MyRootClass>)req).Callback != null)
                ((RequestObject<MyRootClass>)req).Callback(response, resp.Error);
        }
        ...
        private void OnMessageResponse(RESTConnector.Request req, RESTConnector.Response resp)
        {
            ...
            {
                string json = Encoding.UTF8.GetString(resp.Data);
                //Log.Error("AssistantService.OnMessageResponse()", "Json: {0}", json);
        ...
        /// Changes for IOS Builds Testing phase 
        /// 
        /// testing is done every things are working fine, final able to desreialize the data from server in IOS Devices.
        /// 

        public bool MessageIOS(Callback<MyRootClass> callback, string assistantId, string sessionId, MessageInput input = null, MessageContext context = null)
        {
            if (callback == null)
                throw new ArgumentNullException("`callback` is required for `Message`");
            if (string.IsNullOrEmpty(assistantId))
            ...
            req.OnResponse = OnMessageResponseIOS;
        ...
        private void OnMessageResponseIOS(RESTConnector.Request req, RESTConnector.Response resp)
        {
            DetailedResponse<MyRootClass> response = new DetailedResponse<MyRootClass>();
            foreach (KeyValuePair<string, string> kvp in resp.Headers)
            {
            ...
            {
                string json = Encoding.UTF8.GetString(resp.Data);
               // Log.Error("IOS", "Json: {0}", json);
                response.Result = (MyRootClass)Pathfinding.Serialization.JsonFx.JsonReader.Deserialize(json,typeof(MyRootClass));
               // Log.Error("AssistantService.OnMessageResponse() IOS", "response: {0}", response.Result.output.generic[0].text);
