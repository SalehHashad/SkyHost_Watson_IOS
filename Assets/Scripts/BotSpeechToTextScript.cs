...
public class BotSpeechToTextScript : MonoBehaviour
{
    [SerializeField]
    ...
    {
        Debug.Log(gameObject.GetComponent<BotSpeechToTextScript>().settings);
        LogSystem.InstallDefaultReactors();
        Runnable.Run(CreateService());
        status = ProcessingStatus.Idle;

        if (model == IBM_LanguageModels.AR_BroadbandModel)
        {
            languageModel = "ar-" + model;
        }
        else
        {
