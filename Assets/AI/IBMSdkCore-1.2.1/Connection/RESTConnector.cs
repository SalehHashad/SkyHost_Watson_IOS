    ...
    public class RESTConnector
    {
        ...
        private IEnumerator ProcessRequestQueue()
        {
            ...
            while (_requests.Count > 0)
            {
                ...
                if (req.Cancel)
                {
                    Log.Error("req.Cancel()", "req.Cancel", url);

                    Runnable.Run(ProcessRequestQueue()); // IOS Changes for Server Time Out
                ...
                if (!unityWebRequest.isDone)
                {
                    ...
                    {
                        bError = true;
                        Log.Error("unityWebRequest.isDone 625", "Request timed out for URL: {0}", url);
